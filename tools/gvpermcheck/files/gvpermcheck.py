...
def cmd_ssh(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        exit_code, stdout, stderr = ssh_exec(client, SSH_PERM_SCRIPT, sudo=True, password=sudo_password)
        client.close()
...
def cmd_sudoers(args: argparse.Namespace) -> None:
    ...
    try:
        client = ssh_connect(target, strict_hostkey=args.strict_hostkey, password=password, key_path=key_path)
        exit_code, stdout, stderr = ssh_exec(client, SUDOERS_PERM_SCRIPT, sudo=True, password=sudo_password)
        client.close()
...
def cmd_paths(args: argparse.Namespace) -> None:
    ...
    try:
        client = ssh_connect(target, strict_hostkey=args.strict_hostkey, password=password, key_path=key_path)
        exit_code, stdout, stderr = ssh_exec(client, PATHS_PERM_SCRIPT, sudo=True, password=sudo_password)
        client.close()
